...
//        $this->redirect('newUser');
    ...
    public function events(){
		...
		$listRows = C('ADMINEVENTS_LISTROWS');
		$event_count = $event_model->where($where_clause)->count();
		$Page = new Page($event_count,$listRows);
		$event_result = $event_model->relation(true)->where($where_clause)->order('create_time desc')->limit($Page->firstRow.','.$listRows)->select();
    ...
    public function users(){
		...
		$Page = new Page($user_count,$listRows);
    ...
    public function batch(){
        ...
        if($action=='lock'){
            ...
            setflash('ok','',L("您已成功屏蔽所选$model_word"));
        }
        else if($action=='check'){
            $data['is_checked']='1';
            $model->where(array('id'=>array('in',$ids)))->save($data);
            setflash('ok','',L("您已成功审核所选$model_word"));
        }
        else if($action=='recovery'){
            $data['enabled']='1';
            $model->where(array('id'=>array('in',$ids)))->save($data);
            setflash('ok','',L("您已成功恢复所选$model_word"));
        }
        else if($action=='del'){
            $data['enabled']='0';
            $model->where(array('id'=>array('in',$ids)))->save($data);
            setflash('ok','',L("您已成功删除所选$model_word"));
        }
        else if($action=='add_v'){
            $data['is_vip']='1';
            $model->where(array('id'=>array('in',$ids)))->save($data);
            setflash('ok','',L("您已成功加V所选$model_word"));
        }
        else if($action=='del_v'){
            $data['is_vip']='0';
            $model->where(array('id'=>array('in',$ids)))->save($data);
            setflash('ok','',L("您已成功取消加V所选$model_word"));
        }
